...
import { FaChartLine, FaBoxOpen, FaExclamationTriangle } from "react-icons/fa";
  ...
  return (
    <div className="min-h-screen p-6 md:p-8 bg-white text-gray-900">
      <div className="mb-8">
        ...
        <p className="text-gray-600 text-sm">View your sales performance and stock alerts</p>
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-8">
        ...
        >
          <FaChartLine /> <span>My Sales</span>
        </button>
        <button
        ...
        >
          <FaExclamationTriangle /> <span>Low Stock</span>
        </button>
      </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div className="bg-white border border-gray-200 rounded-2xl p-6 hover:border-yellow-400/40 hover:shadow-lg hover:shadow-yellow-400/10 transition-all">
                  <div className="text-gray-600 text-sm mb-2">My Total Sales</div>
                ...
                <div className="bg-white border border-gray-200 rounded-2xl p-6 hover:border-yellow-400/40 hover:shadow-lg hover:shadow-yellow-400/10 transition-all">
                  <div className="text-gray-600 text-sm mb-2">My Transactions</div>
                  <div className="text-3xl font-bold text-gray-900">{salesReport.total_transactions || 0}</div>
                </div>
                <div className="bg-white border border-gray-200 rounded-2xl p-6 hover:border-yellow-400/40 hover:shadow-lg hover:shadow-yellow-400/10 transition-all">
                  <div className="text-gray-600 text-sm mb-2">My Avg Sale</div>
              ...
              {salesReport.transactions && salesReport.transactions.length > 0 && (
                <div className="bg-white border border-gray-200 rounded-2xl p-6">
                  <h3 className="text-xl font-bold text-white mb-4">My Recent Transactions</h3>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {salesReport.transactions.map(tx => (
                          <tr key={tx.id} className="border-b border-gray-200 hover:bg-gray-700/30 transition-all">
                            <td className="py-2 px-4 text-gray-900">{tx.transaction_code}</td>
                            <td className="py-2 px-4 text-center text-gray-700">{tx.items?.length || 0}</td>
                            <td className="py-2 px-4 text-right text-green-400">₱{parseFloat(tx.total).toFixed(2)}</td>
                            <td className="py-2 px-4 text-gray-700">{new Date(tx.created_at).toLocaleString()}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                      ...
                      lowStock.map((product) => (
                        <tr key={product.id} className="border-b border-gray-200 hover:bg-gray-50 transition">
                          ...
                          <td className="py-3 px-4 text-center">
                            <span className={product.stock === 0 ? 'text-red-400 font-bold' : 'text-yellow-400 font-bold'}>
                              {product.stock}
                            </span>
                          </td>
